...
namespace Telemetry_Device.Services.Networking
{
    public class UdpChecksumCalculator : IUdpChecksumCalculator
    {
        public ushort ComputeUdpChecksum(byte[] frame)
        {
            ...
            for (int offset = ConstantPackets.START_IP_HEADER; offset <= ConstantPackets.END_IP_HEADER; offset += ConstantBits.WORD_SIZE_BYTES)
            ...
            for (int udpWordOffset = 0; udpWordOffset < ConstantPackets.FIXED_UDP_LENGTH; udpWordOffset += ConstantBits.WORD_SIZE_BYTES)
            {
